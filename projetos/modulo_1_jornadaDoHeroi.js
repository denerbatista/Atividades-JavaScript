...
do{
    ...
    let resp
    let respS=[]
    console.log(art.style(figlet.textSync('Jornada Do Herói', {font: 'Standard',horizontalLayout: 'default',verticalLayout: 'default',width: 90,whitespaceBreak: false}), "blue"))
    const nome=prompt(art.style("Digite seu nome Herói: ", "white"))
    ...
    console.clear()
    console.log(art.style(`Grande ${nome}! Vamos conferir se todas as contas do mês foram pagas, para ir na pizzaria com a conciência limpa? \nDigite 'sim' ou 'nao':\n`, "white"))
    for(i=0,nivel=0;i<pergs.length;i++){
        resp=prompt(art.style(pergs[i], "white"));
