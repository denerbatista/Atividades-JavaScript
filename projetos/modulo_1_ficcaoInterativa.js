...
function pescar(){
  ...
  if(metodo.random==0){
    status(metodo.i)
    console.log(`\nCaramba não peguei nada só gastei energia e tempo, espero ter mais sorte da próxima vez.`.yellow);
    ...
    console.log(`Peguei umas 3 traíras que dão aproximadamente 900 gramas, estou muito feliz, obrigado Deus !`.yellow)
    prompt("Tecle enter para continuar:")
    metodo.geradorEscolhas(metodo.i,'Estou com muita fome e agora, ASSAR e perder tempo e energia fazendo fogo ou COMER CRU ?',metodo.opcoesAssarCru)
    if(metodo.escolha[0]=="assar"){
...
let metodo={
  ...
  opcoesExplorar:[[' Água ',' Local abrigo ',' Mapa ',' Banana '],['agua','localabrigo','mapa','banana']],
  ...
  escolhaItens:function(){
    for(let i=0;i<2;i++){
      ...
      if(metodo.escolha=='panela'){
        ...
        this.removeAddOpcoes(this.opcoesExplorar,' Madeira e ossos ','madeiraosso',1);
  ...
  construirAbrigo:function(array) {
    ...
    if (this.escolha == "emergencia") {
      console.log(`\nNão gastei muita energia pra fazer, e ficou bom só não sei é resistente a chuvas fortes.`.yellow);
      ...
      console.log(`\nGastei um pouco mais de energia pra fazer, ficou mais resistente mas ainda posso ter problemas com chuvas fortes.`.yellow);
      this.abrigoSemiPermanente=true;
      usuario.perdeEnergia(20);
      this.removeAddOpcoes(this.atividades,' Construir abrigo ','construirabrigo',0)
    } else if (this.escolha == "permanente") {
      console.log(`\nGastei bastante energia pra fazer, ficou muito resistente não terei problemas com chuvas fortes.`.yellow);
    ...
    if(this.escolha=='pescar'){
      if(this.agua==true){
        if(this.pescar==true&&(this.mosquiteiro==true||this.arpao==true)){
          ...
          status(metodo.i)
          console.log('\nTentei pescar, mas não tenho um arpão, uma besta de bambu ou um mosquiteiro que possa usar como rede para pescar, não consegui nada'.yellow);
          usuario.perdeEnergia()
        }   
      }
      ...
      if(this.cacar==true&&(this.besta==true||this.arpao==true)){
        ...
        status(metodo.i)
        console.log('\nTentei caçar, mas não tenho um arpão ou uma besta de bambu para capturar um animal, não consegui nada'.yellow);
        usuario.perdeEnergia()
      }   
  ...
  funcaoColetar:function(){
    ...
    if(this.random==0){
      status(this.i)
      console.log(`\nCaramba não consegui pegar as bananas, esses macacos são muito violentos, e estavam vigiando a moita.`.yellow);
      usuario.perdeEnergia();
    }else if(this.random==1){
      metodo.comeu=true;
      status(this.i);
      console.log(`\nConsegui driblar os macacos e pegar algumas bananas estão um delicia.`.yellow);
  ...
  funcaoCacar:function(){
    ...
    if(this.random==0){
      status(this.i)
      console.log(`\nCaramba não consegui nada só gastei energia e tempo, espero ter mais sorte da próxima vez.`.yellow);
      usuario.perdeEnergia();
    }else if(this.random==1){
      status(this.i);
      console.log(`\nPeguei um animal de porte pequeno.`.yellow);
      prompt('tecle enter para continuar');
      this.geradorEscolhas(this.i,'Estou com muita fome e agora, ASSAR e perder tempo e energia fazendo fogo ou COMER CRU ?',this.opcoesAssarCru)
      ...
      console.log(`\nPeguei um animal de porte médio, estou muito feliz, obrigado Deus !`.yellow)
      prompt("Tecle enter para continuar:")
      this.geradorEscolhas(this.i,'Estou com muita fome e agora, ASSAR e perder tempo e energia fazendo fogo ou COMER CRU ?',this.opcoesAssarCru)
      if(this.escolha[0]=="assar"){
  ...
  conseguirAgua:function(){
    ...
    if(this.panela==true||this.panelaBambu==true){
      this.geradorEscolhas(this.i,'Estou com muita sede, mas será que devo ferver antes ?',this.opcaoSimNao)
      status(metodo.i)
      if(this.escolha=='sim'){
        this.alteraOpcoesTemp('beber água purificada',10,'beberagua'.yellow)
        this.fazerFogo(this.opcoesTemp)
      ...
      console.log(`\nTerei que beber assim mesmo sem purificar não tenho panela para ferver, mas minha saúde irá diminuir.`.yellow);
      usuario.alteraSaude();
    }
  ...
  explorarArea:function(){
    ...
    if(this.random=='agua'){
      ...
      this.bambu=true;
      this.removeAddOpcoes(this.atividades,' Construir ferramentas ','construirferramentas',1);
      this.removeAddOpcoes(this.opcoesFerramentas,' Besta de bambu ','bestadebambu',1);
      if(this.panela==false){
        this.removeAddOpcoes(this.opcoesFerramentas,' Panela de bambu ','paneladebambu',1);
      }
      this.removeAddOpcoes(this.opcoesExplorar,' Bambu ','bambu',0);
    }else if(this.random=='madeiraosso'){
      console.log(`\nEncontrei madeira e ossos posso construir um arpão com isso.`.yellow);
      this.madeiraosso=true;
      this.removeAddOpcoes(this.atividades,' Construir ferramentas ','construirferramentas',1);
      this.removeAddOpcoes(this.opcoesFerramentas,' Arpão ','arpao',1);
      this.removeAddOpcoes(this.opcoesExplorar,' Madeira e ossos ','madeiraosso',0);
    }else{
  ...
  construirFerramentas:function(){
    if(this.opcoesFerramentas[0].length!=0){
        this.geradorEscolhas(this.i,`Quais dessas ferramentas irei construir: ${this.opcoesFerramentas[0]}?`,this.opcoesFerramentas);
        status(this.i)
        if(this.escolha[0]=='arpao'){
            ...
            this.cacar=true;
            this.removeAddOpcoes(this.opcoesFerramentas,' Besta de bambu ','bestadebambu',0);
            this.removeAddOpcoes(this.atividades,' Buscar alimento ','buscaralimento',1);
            this.removeAddOpcoes(this.opcoesAlimentos,' Caçar ','cacar',1);
            console.log(`\nMuito bom agora com essa besta de bambu posso pescar ou caçar.`.yellow)
        }else if(this.escolha[0]=='paneladebambu'){
            this.panelaBambu=true;
            this.removeAddOpcoes(this.opcoesFerramentas,' Panela de bambu ','paneladebambu',0);
            console.log(`\nMuito bom agora com essa panela posso purificar minha água, nem fez falta eu não ter trazido a minha.`.yellow)
        }
    }else{
        console.log(`\nTentei construir mais alguma coisa mas não tenho matéria prima nem ideias, acho que já deu de ferramentas por enquanto.`.yellow);
        this.removeAddOpcoes(this.atividades,' Construir ferramentas ','construirferramentas',0);
    }
},
  facao:false,
  arpao:false,
  ...
  fazerFogo: function fazerFogo(array) {
    if (this.pederneira == true) {
      status(metodo.i)
      console.log(`\nCom essa pederneira é mole fazer fogo, agora posso ${array[0]}.`.yellow);
      usuario.perdeEnergia(5);
      usuario.ganhaEnergia(array[1]);
    } else if (this.pederneira == false) {
      status(metodo.i)
      console.log(`\nNão tenho pederneira para fazer fogo terei que tentar por fricção isso nem sempre da certo.`.yellow);
      prompt('tecle enter para continuar.');
      this.geradorRandom(2);
      if (this.random == 0) {
        status(metodo.i)
        ...
        status(metodo.i);
        console.log(`\nNão foi fácil conseguir uma brasa, mas consegui, da até vontade de chorar agora posso ${array[0]}.`.yellow);
        usuario.perdeEnergia(20);
        usuario.ganhaEnergia(array[1]);
      }
    }
  ...
  comerCru:function(){
    ...
    metodo.geradorRandom(2);
      if(metodo.random==0){
        console.log(`Não sei se foi uma boa escolha comer sem assar estou com dor de barriga !`.yellow)
        usuario.alteraSaude(20,0);
        usuario.perdeEnergia();
        usuario.alteraAsp(0);
      }else if(metodo.random==1){
        console.log(`O sabor não é muito bom, mas pelo menos acumulei alguma energia e por sorte não tive dor de barriga.`.yellow);
  ...
  dormir:function(){
    ...
    if(this.localAbrigo==false){
      ...
      if(this.abrigoPermanente==true){
        console.log(`\nSerá ótimo um descanso, ainda mais neste abrigo permanente que fiz, não preciso temer chuvas`.yellow);
        prompt('tecle enter para continuar:')
        this.verificaMosquito();
      }else if(this.abrigoSemiPermanente==true){
        console.log(`\nSerá ótimo um descanso, meu abrigo semi-permanente é bom porém não é reforçado contra chuvas mais fortes.`.yellow);
        prompt('tecle enter para continuar:')
        this.verificaMosquito();
        this.chuva(this.abrigoSemiPermanente,3);
      }else if(this.abrigoEmergencial==true){
        console.log(`\nSerá ótimo um descanso, mas meu abrigo emergencial não é páreo contra chuvas mais fortes.`.yellow);
        prompt('tecle enter para continuar:')
        this.verificaMosquito();
        this.chuva(this.abrigoEmergencial,2);
      }
    }else{
      console.log(`\nDormir fora de um abrigo não vai ser legal minha saúde vai diminuir.`.yellow);
  ...
  verificaNoiteSede:function(){
    if(this.agua==true&&usuario.hidratacao<60){
        this.removeAddOpcoes(metodo.atividades,' Conseguir agua ','conseguiragua',1)
     }else if(this.agua==true&&usuario.hidratacao==60){
        this.removeAddOpcoes(metodo.atividades,' Conseguir agua ','conseguiragua',0)
     }
    if(metodo.j==2){
  ...
  verificaMosquito:function(){
    if(this.mosquitos==true){
      this.geradorEscolhas(this.i,'Não tenho mosquiteiro preciso conter esses insetos, eles irão atrapalhar meu sono será que tento fazer fogo pra afastá-los ou não ?',this.opcaoSimNao);
      if(this.escolha=='sim'){
        metodo.alteraOpcoesTemp('dormir em paz',10,'dormir');
        this.fazerFogo(this.opcoesTemp);
        usuario.alteraAsp(1);
      }else if(this.escolha=='nao'){
        console.log(`Vou tentar suportar esses mosquitos, mas já sei que perderei energia fazendo isso.`)
        usuario.alteraAsp(0);
        usuario.perdeEnergia(10);
      }
    }else{
      console.log(`Pelo menos não terei problemas com insetos com esse mosquiteiro`.yellow);
      usuario.alteraAsp(1);
      usuario.ganhaEnergia(10);
    }
  },
  pedirAjudaMedica:function(){
    status(this.i);
    console.log("\nO médico irá avaliar se pode me ajudar aqui, ou se vou precisar desistir para ir a um hospital".yellow);
    prompt("tecle enter para continuar");
    this.geradorRandom(2,'normal');
    if(this.random==0){
      this.geradorEscolhas(this.i,'O médico me orientou a desistir e ir no hospital me tratar e agora CONTINUAR ou DESISTIR ? \nSe eu continuar posso até conseguir chegar no dia 21, mas minhas atividades terão mais chance de erro por conta da minha condição.',metodo.opcaoContinuaDesiste)
      if(this.escolha[0]=='desistir'){
        ...
        status(this.i);
        console.log("\nOk vou continuar mas a vida não vai ser fácil pra mim daqui por diante, mas vou continuar até não aguentar mais.".yellow);
  ...
  chuva:function(tipoAbrigo,qtdRandom){
    ...
    if(this.random==0){
        console.log('Houve uma chuva a noite meu abrigo não resistiu'.yellow);
        tipoAbrigo=false;
        this.removeAddOpcoes(this.atividades,' Construir abrigo ','construirabrigo',1);
    }
},
resultado:function(motivo){
  if(metodo.i<7){
      ...
      console.log(`\n${motivo} e precisei sair do programa fiquei apenas ${metodo.i} dias, perdi ${80-usuario.peso} e minha ASP fechou em ${usuario.asp}.`.yellow)
  }else if(metodo.i==7&&this.mapa==true){
      status(metodo.i,'Vencedor',1)
      console.log(`\nVenci cheguei ao 7º dia, tenho tudo para sobreviver largado e pelado! Perdi ${80-usuario.peso}Kg e minha ASP fechou em ${usuario.asp}`.yellow)
  }else{
    status(metodo.i,'Perdedor',1)
    console.log(`\nCheguei ao 7º dia, mas não encontrei o mapa para o local de resgate, não acredito! Perdi ${80-usuario.peso}Kg e minha ASP fechou em ${usuario.asp}`.yellow)
  }
},
}
...
let usuario = {
  ...
  alteraSaude:function(valor=10,restaurar=0,valor2=100){
    if(restaurar==1){
      ...
      if(this.saude<50&&this.negarDesistencia==false){
        metodo.geradorEscolhas(metodo.i,`Minha saúde está baixa será que devo pedir ajuda ${metodo.opcaoSimNao[0][0]} ou ${metodo.opcaoSimNao[0][1]} ? `,metodo.opcaoSimNao)
        if(metodo.escolha[0]='sim'){
          metodo.pedirAjudaMedica()
        }else if(this.saude<=0){
          prompt('tecle enter para continuar')
          status(metodo.i);
          console.log(`\nNão suporto mais minha saúde chegou no limite estou morrendo, produção me ajuda por favor!`.red);
    ...
    if(valor==1){
      ...
      if(this.hidratacao>=20){
        ...
        if(this.hidratacao<=0){
          console.log(`Sinto caimbras não consigo ficar de pé estou a três dias sem água, produção por favor me ajuda!`.red)
          metodo.escolha='Fiquei desidratado'
  ...
  verificaComeuBebeuDormiu:function(){
    ...
    if(metodo.bebeuAgua==true){
        ...
        console.log('\nPassei o dia sem tomar água, se fico três dias seguidos sem me hidratar não sobrevivo.'.yellow);
...
metodo.escolhaItens()
  ...
  metodo.resultado(metodo.escolha)
  loop=(prompt("Digite 'R' para reiniciar o jogo ou qualquer tecla para finalizar: ").toLowerCase()).replace(/ /g, '');
}while(loop=='r')    
